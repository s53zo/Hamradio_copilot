...
# Zone coordinates and distance calculation
zones = {
    1: (61.2181, -149.9003),
    2: (46.8139, -71.2080),
    3: (34.0522, -118.2437),
...
distance_matrix = [[0.0 for _ in range(40)] for _ in range(40)]
for i in range(1, 41):
    for j in range(1, 41):
        if i == j:
            ...
            distance_matrix[i-1][j-1] = round(distance, 2)
...
def calculate_expected_attenuation(to_zone):
    """
    Calculate expected signal attenuation based on distance from Zone 15,
...
def run(access_key=None, secret_key=None, s3_buck=None, include_solar_data=False):
    ...
    # Add Scale column to combined_table
    combined_table['Scale'] = ''  # Empty values, we'll use color_styles for coloring
    
    # Add Propagation descriptions
    for idx, row in mean_table.iterrows():
